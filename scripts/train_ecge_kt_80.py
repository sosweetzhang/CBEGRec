"""Train KT model on 80% data (simulator / ground-truth student state). Saves to models/{domain}/ecge_kt_80/."""
import sys
...
class NoneNegClipper(object):
    """Clip output layer weights to be non-negative."""
...
def parse_device(device_str: str) -> torch.device:
    """Parse device string to torch.device (auto, cpu, cuda, cuda:0, 0, 1, ...)."""
...
class ECGEKT80Trainer:
    """Train KT (80% data) for simulator."""
    DATA_SPLIT_SEED = 42
    
    def __init__(self, config_path: str = None, device: str = None):
        self.config = load_config(config_path)
        self.logger = AppLogger("ECGE_KT_80_Trainer", level=self._get_log_level())
        ...
        self.logger.info(f"Training KT model with {self.data_ratio*100:.0f}% of data")
        ...
        self.model_path = base_model_dir / self.domain / "ecge_kt_80"
        self.model_path.mkdir(parents=True, exist_ok=True)
        self.logger.info(f"Model will be saved to: {self.model_path}")
        checkpoint_dir = Path(self.config['model']['checkpoint_dir']) / self.domain / "ecge_kt_80"
        ...
        self.logger.info("ECGE KT 80% Trainer initialized")
    ...
    def _load_data(self):
        ...
        indices_file = self.model_path / "sampled_indices_80.json"
    ...
    def train(self, resume_from_checkpoint: str = None):
        ...
        self.logger.info("Starting training (KT 80%)...")
    ...
    def _save_model(self, epoch: int, auc: float, is_best: bool = False):
        ...
        with open(details_path, 'w', encoding='utf-8') as f:
            f.write(f'Model Type: KT (Simulator)\n')
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train E-CGE KT Model (80% data)")
    ...
    print("=" * 60)
    print("Training E-CGE KT Model (80% data) for Simulator")
    print("=" * 60)
    
    trainer = ECGEKT80Trainer(config_path=args.config, device=args.gpu)
