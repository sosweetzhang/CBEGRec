...
class EnvDKTtrainer:
    def __init__(self, num_skills, train_goal='env_DKT'):
        ...
        self.base_data_path = str(_PROJ_ROOT / "data" / "ASSIST19" / "processed")
        self.agent_dkt_data_path = str(_PROJ_ROOT / "data" / "dataProcess" / "offline")
        ...
        if self.train_goal == 'env_DKT':
            self.weight_path = str(_PROJ_ROOT / "EduSim" / "Envs" / "KES_ASSIST09" / "meta_data" / "env_weights")
            os.makedirs(self.weight_path, exist_ok=True)
        elif self.train_goal == 'agent_DKT':
            self.weight_path = str(_PROJ_ROOT / "EduSim" / "Envs" / "KES_ASSIST09" / "meta_data" / "agent_weights")
            os.makedirs(self.weight_path, exist_ok=True)
        ...
        self.train_dataset = Dataloader(source=self.train_dataset, column_names=['session'])
        self.train_dataset = self.train_dataset.batch(self.batch_size)

        self.val_dataset = Dataloader(source=self.val_dataset, column_names=['session'])
        self.val_dataset = self.val_dataset.batch(self.batch_size)

        self.test_dataset = Dataloader(source=self.test_dataset, column_names=['session'])
    ...
    def compute_loss(self, output, batch_data):
        ...
        for episode in range(batch_data.shape[0]):
            ...
            target_id = torch.where(tmp_target_id > self.num_skills - 1,
                                            tmp_target_id - self.num_skills, tmp_target_id)  
